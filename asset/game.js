...
class DiveWorld {
    ...
    updateInput(name, value) {
        if (this.outputList[name]) {
            this.outputList[name].value = value;
            console.log(`Updated input: ${name} = ${value}`);
        } else {
            console.warn(`Input name "${name}" not found in outputList.`);
        }
    }
 ...
 */
let externalVariable; // 假設這是外部變數

// 從 localStorage 中獲取 username
const username = localStorage.getItem('username');
if (username) {
    console.log("Username:", username);
    // 設定為變數 externalVariable
    externalVariable = username;
} else {
...
/***************************以下是地圖&地圖參數設定***************************/
const map_col = 3; //地圖行
const map_row = 10; //地圖列
const map = [
    33172, 33151, 33151, // 主畫面； APD : 01 準備物品 00
    33152, 33153, 33154, //02 準備環境 10,11,12
    33155, 33156, 33156,   // 03_00 接引流管 20,21,22
    33157, 33158, 33159, //03_01 接透析液管 30,31,32
    33160, 33161, 33162, // 03_02 接補充液袋 40,41,42 還沒切換過去

    33163, 33164, 33165,// 04 接病人端管路 50,51 洗手,52
    33166, 33167, 33168, // 04_01 連接病人端管路_小指 60,61,62
    33169, 33150, 33150,// 05 分離管組 70,71,72
    33150, 33150, 33150,// 06 分離管組與紀錄 80,

    33144, 33144, 33144, // 測驗題 90 新版本

    32099, 32100, 33194, // CAPD: 10_0 新版本250122
    32102, 32103, 32103, // 11
    33196, 33196, 32105, // 12

    33173, 33174, 33175, // 13 PDCC
    33176, 33177, 33178, // 14
    33179, 33180, 33181, // 15
    33182, 33146, 33146  //16
];
/**
 * 「指定預載」 
...
diveWorld.updateInput("UserID", externalVariable);